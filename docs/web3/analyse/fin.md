...
## 滑点 (Slippage)
- **介绍**: 指交易订单在下单和实际成交之间，由于市场价格快速变动，导致成交价格与预期价格之间存在差异的现象。滑点可能是正向的（成交价格优于预期），也可能是负向的（成交价格劣于预期），在流动性较低或市场波动剧烈时更为常见。滑点在使用市价单时尤为突出，因为市价单追求快速成交，而非价格控制。

## AMM (Automated Market Maker)
- **介绍**: 自动做市商是一种去中心化交易机制，它不依赖订单簿，而是通过数学公式自动决定交易价格，常见如 Uniswap 使用的恒定乘积公式（x × y = k）。交易者与资金池交互，通过添加或移除流动性完成兑换操作。

## Restaking（再质押）
- **介绍**: 再质押是一种让用户在已有质押资产的基础上，将其“复用”以支持更多网络或协议的安全、验证或功能的机制，从而获得额外收益。Restaking 通常不会撤回原质押，而是在保持原链权益的同时，用该权益再支持其它系统，典型代表是 EigenLayer（以太坊生态）。
